...
const locale = {
  title:
    "A Budapest V. kerületi Eötvös József Gimnázium Diákönkormányzat honlapjának Adatkezelési Tájékoztatója",
  date: "2023 október",
  laws: {
...
const PrivacyPolicyPage = () => {
  return (
    <div>
      ...
      <table>
        <tr>
          <th scope="row">{locale.participants.handler.headers.name}</th>
          <td>{locale.participants.handler.values.name}</td>
        </tr>
        <tr>
          <th scope="row">{locale.participants.handler.headers.address}</th>
          <td>{locale.participants.handler.values.address}</td>
        </tr>
        <tr>
          <th scope="row">{locale.participants.handler.headers.phone}</th>
          <td>{locale.participants.handler.values.phone}</td>
        </tr>
        <tr>
          <th scope="row">{locale.participants.handler.headers.email}</th>
          <td>
            <Link to={`mailto:${locale.participants.handler.values.email}`}>
              {locale.participants.handler.values.email}
            </Link>
          </td>
        </tr>
        <tr>
          <th scope="row">{locale.participants.handler.headers.website}</th>
          <td>
        ...
        <tr>
          ...
          <td>{locale.participants.handler.values.activity_location}</td>
        </tr>
        <tr>
          <th scope="row">
            {locale.participants.handler.headers.storageprovider}
          </th>
          <td> {locale.participants.handler.values.storageprovider}</td>
        </tr>
      </table>
